...
Uses FBInk TrueType rendering + wttr.in plain text format (no JSON parsing needed).
"""
import tarfile
import io
import os
...
# Prevent Kobo from going to sleep (holds a kernel wakelock)
echo "ebook-dash" > /sys/power/wake_lock 2>/dev/null

# Quick WiFi reconnect (no retries — keep it fast so clock doesn't freeze)
ensure_wifi() {
...
# Draw text in a bounded box (non-overlapping regions prevent clipping)
# Usage: ttbox SIZE TOP BOTTOM LEFT RIGHT "text" [bold]
ttbox() {
...
# HTTPS fetch with openssl fallback (busybox wget lacks SSL)
fetch_https() {
    _URL="$1"
    # Method 1: curl
    ...
    # Method 2: openssl s_client (follow redirects)
